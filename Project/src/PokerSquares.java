...
public class PokerSquares {
	...
	public static double[] playTournament(
		...
		for (PokerSquaresPointSystem system : systems) { // for each point system
			...
			for (int i = 0; i < players.size(); i++) { // for each player
				double normalizedTotal = (double) (totalScores[i] - minTotal + 1) / (maxTotal - minTotal + 1);
	...
	private static double[] tournamentScores;
	private static int[] totalScores;
	private static ArrayList<Runnable> threads;
	private static ExecutorService executor;
	
	public static double[] playMultiThreadedTournament(
			ArrayList<PokerSquaresPlayer> players,
			PokerSquaresPointSystem pointSystem,
			int gamesPerSystem, long startSeed) {
		
		
		tournamentScores = new double[players.size()];
		...
		totalScores = new int[players.size()];
		
		long e = System.currentTimeMillis();
		// thread portion
		for (int i = 0; i < players.size(); i++) { // for each player
				
			threads.add(new PokerSquaresWorkerThread(players.get(i), pointSystem, i, gamesPerSystem, startSeed));
	...
	public static class PokerSquaresWorkerThread implements Runnable {
		...
		public PokerSquaresWorkerThread(PokerSquaresPlayer player, PokerSquaresPointSystem system, int playerID, int gamesPerPlayer, long startSeed){
			this.player = player;
			this.system = system;
			this.id = playerID;
		...
		public void run(){
			...
			//System.out.printf("Player: \"%s\"\n", player.getName());
			this.scores = new PokerSquares(this.player, this.system).playSequence(this.gamesPerPlayer, this.startSeed, false);


			for (int score : this.scores){
				synchronized(totalScores){
					totalScores[this.id] += score;
				}
				//System.out.printf("Player \"%s\" total score: %d\n", player.getName(), totalScores[this.id]);
			}
...
//                systems.add(PokerSquaresPointSystem.getHypercornerPointSystem());  // each hand type scored as -1 or 1, with equal probability
                ...
                PokerSquares.playMultiThreadedTournament(players, systems.get(0), 100, 0L);  // play 10 games for each player under each scoring system
